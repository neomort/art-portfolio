/*
  # Fix Calendar Attachment Function

  1. New Functions
    - `create_booking_calendar_attachment_v4` - Renamed version of the function with correct parameter order
    - `create_ics_calendar_content_v4` - Improved version with better error handling

  2. Changes
    - Renames the function to force PostgREST to refresh its schema cache
    - Ensures parameters are in the correct order to match function calls
    - Adds additional logging for better debugging
*/

-- Create an improved ICS content generator with proper formatting
CREATE OR REPLACE FUNCTION public.create_ics_calendar_content_v4(
  event_title TEXT,
  start_date DATE,
  end_date DATE,
...
-- Create a renamed version of the attachment creator with parameters in the correct order
...
COMMENT ON FUNCTION public.create_ics_calendar_content_v4 IS 'Creates ICS calendar content with proper formatting and sanitization (v4)';
COMMENT ON FUNCTION public.create_booking_calendar_attachment_v4 IS 'Creates a complete ICS attachment for booking confirmations with enhanced error handling and logging (v4)';