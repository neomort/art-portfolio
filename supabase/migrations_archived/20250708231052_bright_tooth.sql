/*
  # Fix ICS Calendar Data Issues

  1. New Functions
    - `create_booking_calendar_attachment_v6` - Improved calendar attachment generator with:
      - Correct date handling
      - Complete location information (street, city, state, postal code, country)
      - Specific booking URL in description
      - Better error handling and logging

  2. Security
    - All functions use SECURITY DEFINER to ensure they run with appropriate permissions
*/

-- Create a new version of the ICS content generator with improved description URL
...
BEGIN
  ...
  IF booking_id IS NOT NULL THEN
    dashboard_url := 'https://splitspace.app/dashboard?booking=' || booking_id;
  ELSE
    dashboard_url := 'https://splitspace.app/dashboard';
  END IF;
  
  -- Update description to include specific booking URL if not provided
  IF description IS NULL OR description = '' THEN
    description := 'Your booking confirmation from SplitSpace. View details at ' || dashboard_url;
  END IF;
...
-- Create a new version of the calendar attachment function with complete location and specific booking URL
...
BEGIN
  ...
  description := 'Your booking confirmation from SplitSpace. View details at https://splitspace.app/dashboard?booking=' || booking_id;
...
COMMENT ON FUNCTION public.create_ics_calendar_content_v5 IS 'Creates ICS calendar content with proper formatting, sanitization, and specific booking URL (v5)';
COMMENT ON FUNCTION public.create_booking_calendar_attachment_v6 IS 'Creates a complete ICS attachment for booking confirmations with complete location and specific booking URL (v6)';