...
const SignUpPage: React.FC = () => {
  ...
  // Prefill from query params when present (e.g., coming from InquiryForm guest flow)
  useEffect(() => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('already registered') || errorMessage.includes('User already registered') || errorMessage.includes('duplicate key value') || errorMessage.includes('email already exists') || errorMessage.includes('Email rate limit exceeded') || errorMessage.toLowerCase().includes('user with this email')) {
